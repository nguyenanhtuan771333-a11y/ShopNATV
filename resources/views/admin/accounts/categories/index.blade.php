          ...
          <tbody>
            @foreach ($categories as $item)
              <tr>
                ...
                <td>
                  <div class="d-flex">
                    <a href="javascript:void(0)" class="badge bg-primary-gradient me-1" data-bs-toggle="modal" data-bs-target="#modal-edit-{{ $item->id }}"><i class="fa fa-edit"></i> sửa</a>
                    <a href="{{ route('admin.accounts.groups', ['id' => $item->id]) }}" class="badge bg-info-gradient me-1"><i class="fa fa-eye"></i> xem</a>
  ...
  <script>
    const deleteRow = async (id) => {
      ...
      try {
        const {
          data: result
        } = await axios.post('{{ route('admin.accounts.categories.delete') }}', {
          id
        })
