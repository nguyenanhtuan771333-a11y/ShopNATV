...
<x-app-layout>
  ...
  <div class="grid grid-cols-1 gap-3 md:grid-cols-3">
    @foreach ($accounts as $account)
      <div class="grid grid-cols-3 items-center rounded-lg bg-transparent bg-cover text-white cursor-pointer" style="background-image: url(/images/all-img/widget-bg-3.png)">
        ...
        <a href="{{ route('account.orders.accounts', ['code' => $account->code]) }}" class="flex flex-col justify-between whitespace-nowrap text-truncate col-span-2 p-2 text-[16px] font-bold">
          <div class="text-truncate">{{ __t('Mã Số:') }} <span class="text-[#A8DF8E]">#{{ $account->code }}</span></div>
          <div class="text-truncate">{{ __t('Mã Đơn:') }} <span class="text-[#EBE76C]">{{ $account->buyer_code }} / {{ Helper::formatCurrency($account->buyer_paym) }}</span></div>
          <div class="text-truncate">{{ __t('Tài Khoản:') }} <span class="text-[#40F8FF]">{{ $account->username }}</span></div>
          <div class="text-truncate">{{ __t('Ngày Mua:') }} <span class="text-[#85E6C5]">{{ $account->buyer_date }}</span></div>
        </a>
      </div>
    @endforeach
  </div>
