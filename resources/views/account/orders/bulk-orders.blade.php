        ...
        <div class="flex-1">
          <div class="max-w-[180px]">
            <h4 class="mb-2 text-2xl font-medium text-white">
              <span class="block text-sm">{{ __t('Đơn Hàng Đã Mua,') }}</span>
  ...
  <div class="grid grid-cols-1 gap-3 md:grid-cols-3 ">
    @foreach ($accounts as $account)
      <div class="grid grid-cols-3 items-center rounded-lg bg-transparent bg-cover text-white" style="background-image: url(/images/all-img/widget-bg-3.png)">
        <a href="{{ route('account.orders.bulk-orders', ['code' => $account->code]) }}" class="p-2">
        ...
        <a href="{{ route('account.orders.bulk-orders', ['code' => $account->code]) }}" class="flex flex-col justify-between whitespace-nowrap text-truncate col-span-2 p-2 text-[16px] font-bold">
          <div class="text-truncate">{{ __t('Có:') }} <span class=" text-[#F4CE14]">{{ $account->orders->count() }} {{ __t('tài khoản') }}</span></div>
          <div class="text-truncate">{{ __t('Mã Đơn:') }} <span class="text-[#E1AA74]">{{ $account->code }} / {{ Helper::formatCurrency($account->buyer_paym) }}</span></div>
          <div class="text-truncate">{{ __t('Tài Khoản:') }} <span class="text-[#40F8FF]">{{ $account->username }}</span></div>
          <div class="text-truncate">{{ __t('Ngày Mua:') }} <span class="text-[#80B3FF]">{{ $account->created_at }}</span></div>
        </a>
      </div>
    @endforeach
  </div>
