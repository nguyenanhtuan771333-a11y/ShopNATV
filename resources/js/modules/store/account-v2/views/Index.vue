...
const buyItem = async (item) => {
  ...
  if (!isBulk) {
    const confirm = await Swal.fire({
      ...
      text: `${$__t('Bạn sẽ mua nick')} #${item.code} ${$__t('với giá')} ${
        item.price_str
      }?`,
  ...
  if (isBulk) {
    const { value: oQuantity } = await Swal.fire({
      icon: 'warning',
      input: 'number',
      inputLabel: __t('Số lượng tài khoản muốn mua'),
      inputPlaceholder: __t('Nhập số lượng cần mua'),
      inputValidator: (value) => {
        if (!value) {
          return __t('Vui lòng nhập số lượng tài khoản cần mua')
        }

        if (parseInt(value) < 1) {
          return __t('Số lượng tài khoản cần mua phải lớn hơn 0')
        }

        if (parseInt(value) > item.is_bulk) {
          return (
            __t('Số lượng tài khoản cần mua không được lớn hơn') +
            ' ' +
            item.is_bulk
          )
        }
  ...
  try {
    ...
    Swal.fire('Great!', result.message, 'success').then(() => {
      if (result.data.is_bulk) {
        window.open('/account/orders/bulk-orders/' + result.data.code, '_self')
      } else {
        window.open('/account/orders/accounts-v2/' + result.data.code, '_self')
      }
    })
  } catch (error) {
    Swal.fire('Oops...', $catchMessage(error), 'error')
...
<template>
  <section>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          <div>
            ...
            <a-input
              ...
              :placeholder="__t('Tìm kiếm mã sản phẩm, trang phục,...')"
              style="width: 100%"
            />
          </div>
          <div class="grid grid-cols-2 gap-3">
            ...
            <div>
              <button
                class="btn btn-sm btn-danger w-full lg:mt-[32px]"
                type="button"
                @click="resetFilter()"
              >
      ...
      <a-row v-if="!loading" :gutter="18">
        ...
        >
          ...
          >
            ...
            >
              <div
                class="border border-primary rounded-lg p-[1px] bg-white dark:bg-gray-800"
              >
                <a-image
                  :src="item.image"
                  :alt="item.name"
                  width="100%"
                  class="rounded-t-lg sm:!h-[200px] lg:!h-[170px]"
                />
                <div
                  class="text-center text-lg font-bold bg-primary text-white"
                ...
                <div class="p-3">
                  <div class="grid grid-cols-2 mb-3 gap-3">
                    <div
                      v-for="(hl, hlx) in item.highlights"
                      :key="hlx"
                      class="lg:col-span-1"
                    >
                      <a-tag
                  ...
                  <div class="text-center grid grid-cols-2 gap-3">
                    <div class="col-span-2">
                      <div
                        class="border border-red-500 rounded-lg p-1 font-bold dark:text-white"
                      >
                        {{ __t('Còn lại') }}
                      ...
                      <div
                        v-if="item.is_bulk > 1"
                        class="border border-red-500 rounded-lg p-1 font-bold dark:text-white mt-2"
                      >
                        {{ __t('Được mua') }}
                        <span class="text-red-500">{{ item.is_bulk }}</span>
                        {{ __t('tài khoản / lần') }}
                      </div>
                    </div>
                    <div>
                      <a-button
                        ...
                        ><del>{{ formatCurrency(item.price) }}</del></a-button
                      >
                      <a-button
                        type="dashed"
                        danger
    ...
    <a-card v-if="products?.length === 0 && !loading">
      <a-empty
        :description="__t('Không tìm thấy tài khoản nào trong nhóm này')"
      ></a-empty>
    </a-card>
