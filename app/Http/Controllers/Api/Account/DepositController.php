...
class DepositController extends Controller
{
  ...
  public function sendCard(Request $request)
  {
    ...
    $payload = $request->validate([
      ...
      'telco'  => 'required|string|in:VIETTEL,VINAPHONE,MOBIFONE,ZING,GARENA,GARENA2,VNMOBI',
    ...
    try {
      ...
      if (isset($result['trans_id'])) {
        ...
        if (isset($result['status']) && $result['status'] === 1) {
          $client = User::find($card->user_id);
          if ($client === null) {
            return response()->json([
              'status'  => 400,
              'message' => 'Không tìm thấy tài khoản cần nạp',
            ], 400);
          }
          ...
          $client->transactions()->create([
            ...
            'status'         => 'paid',
            'content'        => 'Nạp thẻ thành công #' . $card->serial . '; phí ' . $fees . '%',
            'user_id'        => $client->id,
