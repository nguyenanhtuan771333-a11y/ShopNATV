...
class ItemController extends Controller
{
  public function index(Request $request)
  {
    ...
    $claimedOrders    = ItemOrder::where('assigned_to', $user->username)
      ...
      ->orderBy('created_at', 'desc')
      ->paginate(10, ['*'], 'claimed');
    $processingOrders = ItemOrder::where('assigned_to', $user->username)
      ->where('status', 'Processing')
      ->orderBy('created_at', 'desc')
      ->paginate(10, ['*'], 'processing');

    return view('staff.orders.items', compact('pendingOrders', 'claimedOrders', 'processingOrders', 'user'));
  }
